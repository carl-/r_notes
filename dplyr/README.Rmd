...
```{r}
# In dplyr
data %>% select(ID, TIME) # Select columns
data %>% select(ID:MDV)   # Range of columns
...
```{r}
# In dplyr
data %>% group_by(ID) %>% summarize(Cmax = max(DV),
                                    Nobs = n())
...
### `mutate()`
Add or edit columns. See also `transmute()` and `mutate_each()`.
```{r}
# In dplyr
data %>% mutate(TIME_H = TIME_D*24)
